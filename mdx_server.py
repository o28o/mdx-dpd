...
# 新线程执行的代码
def loop(http_port: int = 8000):
    # 创建一个服务器，IP地址为空，端口是8000，处理函数是application:
    httpd = make_server('', http_port, application)
    print("Serving HTTP on port {}...".format(http_port))
    # 开始监听HTTP请求:
    httpd.serve_forever()
...
if __name__ == '__main__':
    ...
    parser.add_argument("port", nargs='?', const="8000", help="port to serve on")
    ...
    if not os.path.exists(args.filename):
        ...
        t = threading.Thread(target=loop, args=(port,))
        t.start()
